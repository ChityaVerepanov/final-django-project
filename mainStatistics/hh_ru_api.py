...
def hh_api():
    req = requests.get(
        'https://api.hh.ru/vacancies?text=web develop, Web-разработчик, веб разработчик, web разработчик, web programmer, web программист, веб программист, битрикс разработчик, bitrix разработчик, cms разработчик,  joomla разработчик, drupal developer, wordpress developer,&professional_role=96&per_page=10')
    data = req.json()
    items = data['items']
